...
//		String SingID = "";
		int log=0;
		String []SingID=new String[30];
//		String SingPW = "";
		String []SingPW=new String[30];
		String LoginID = "";
		String LoginPW = "";
		String Name = "";
//		String Name1 = "";
		...
		while (run) {
			...
			if (login == false) {
				...
				System.out.printf("1.정보수정 2.shop 3.은행 4.game 5.내 정보 6.로그아웃 0.종료%n%s님 잔액 : %,d 원 무기 %s 선택 : ", Name,Banking[i], BoxNam);
			}
			}
			if (sc.hasNextInt()) {
				menu = sc.nextInt();
			...
			case 1:
				...
				for (int j=0; j<log; j++) {
				System.out.println("--------------------------------------------------------------------------------");
				System.out.printf("%s\t\t%s\t\t%s\t\ttest\t\t%,d\t\t%n",Name1[j],SingID[j],SingPW[j],Banking[j]);
				ACard=false;
				}
				}
				else {
				if (login == false) {
					...
					Name1[log] = sc.nextLine();
					System.out.printf("Id : ");
					SingID[log] = sc.nextLine();
					System.out.printf("Pw : ");
					SingPW[log] = sc.nextLine();
					System.out.println("");
					System.out.println("가입완료");
					System.out.printf("Name : %s %n", Name1[log]);
					System.out.printf("ID : %s %n", SingID[log]);
					System.out.printf("PW : %s %n", SingPW[log]);
					Banking[i]=Money;
					log++;
				}
					...
					LoginPW = sc.nextLine();
					System.out.println();
					if (SingID[i].equals(LoginID)) {
						if (SingPW[i].equals(LoginPW)) {
							...
							System.out.printf("ID : %s %n", SingID[i]);
							System.out.printf("PW : %s %n", SingPW[i]);
						}
			...
			case 3:
				if (login == false) {
					...
					System.out.printf("Id : ");
					SingID[i] = sc.nextLine();
					System.out.printf("Pw : ");
					SingPW[i]= sc.nextLine();
					System.out.println();
					if (LoginID.equals(SingID[i])) {
						if (LoginPW.equals(SingPW[i])) {
							boolean BankRun = true;
							while (BankRun) {
								System.out.println("은행");
								System.out.printf("");
								System.out.printf("1.입금 2.출금 3.잔액 4.홈페이지%n잔액 : %,d 원 선택 : ", Banking[i]);
								Bank = sc.nextInt();
								switch (Bank) {
								case 1:
									...
									if (sc.hasNextLine()) {
										...
										if (Ok.equals(BankCh)) {
											Banking[i] += BankIn;
											System.out.printf("금액 입금 완료 잔액 : %,d 원%n", Banking[i]);
											BankRun = false;
								...
								case 2:
									...
									break;
								case 3:
									System.out.printf("잔액 : %,d원 %n", Banking[i]);
									continue;
								case 4:
